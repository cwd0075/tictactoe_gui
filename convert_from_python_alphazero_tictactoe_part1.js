// convert from python to javascript using codeconvert.ai
...
function main() {
  ...
  let player = 1;
  let state = tictactoe.get_initial_state();
  while (true) {
    console.log(state);
    const valid_moves = tictactoe.get_valid_moves(state);
    console.log(
      "valid_moves",
      valid_moves
    ...
    const [value, is_terminal] = tictactoe.get_value_and_terminated(
      state,
      action
    );
    if (is_terminal) {
